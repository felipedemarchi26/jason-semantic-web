...
public class CentralisedMASLauncherAnt implements MASLauncherInfraTier {
    ...
    protected String replaceMarks(String script, boolean debug) {
        ...
        //TODO Testando
        Config c = Config.get();
        String jena = Config.findJarInDirectory(new File(c.getJasonHome()+"/libs"), "jena");
        lib += "        <pathelement location=\""+jena+"\"/>\n";
        String http = Config.findJarInDirectory(new File(c.getJasonHome()+"/libs"), "http");
        lib += "        <pathelement location=\""+http+"\"/>\n";
        String commons = Config.findJarInDirectory(new File(c.getJasonHome()+"/libs"), "commons");
        lib += "        <pathelement location=\""+commons+"\"/>\n";
        String log4j = Config.findJarInDirectory(new File(c.getJasonHome()+"/libs"), "log4j");
        lib += "        <pathelement location=\""+log4j+"\"/>\n";
        String slf4j = Config.findJarInDirectory(new File(c.getJasonHome()+"/libs"), "slf4j");
        lib += "        <pathelement location=\""+slf4j+"\"/>\n";
        String x = Config.findJarInDirectory(new File(c.getJasonHome()+"/libs"), "x");
        lib += "        <pathelement location=\""+x+"\"/>\n";
        //TODO fim testando
